...
CREATE TABLE bookings (
  ...
  status text CHECK (status = ANY (ARRAY['pending', 'confirmed', 'cancelled'])),
  details jsonb,
...
CREATE TABLE transactions (
  ...
  transaction_type text NOT NULL CHECK (transaction_type = ANY (ARRAY['topup', 'spend', 'transfer', 'booking_payment'])),
  status text DEFAULT 'completed' CHECK (status = ANY (ARRAY['pending', 'completed', 'failed'])),
  description text,
