...
export const TransactionList = ({ limit }: { limit?: number }) => {
  ...
  const totalCount = radarData.reduce((sum, d) => sum + d.count, 0);
  const totalAmount = radarData.reduce((sum, d) => sum + d.amount, 0);

  const radarDataPercent = radarData.map((d) => ({
    type: d.type,
    count: totalCount ? (d.count / totalCount) * 100 : 0,
    amount: totalAmount ? (d.amount / totalAmount) * 100 : 0,
  }));
  ...
  if (loading) {
    return (
      <section className="container px-4 mx-auto flex flex-col gap-2">
  ...
  return (
    <section className="container px-4 mx-auto flex flex-col gap-2">
      <Card className="px-2 py-2 shadow-none border-0">
        <CardContent className="px-0">
          <ChartContainer
            config={chartConfig}
            className="mx-auto w-full aspect-square"
          >
            <RadarChart data={radarDataPercent}>
                  ...
                  return (
                    ...
                    >
                      <div className="h-12 w-12 flex items-center justify-center rounded-md bg-accent/40">
                        {tx.type === "topup" && (
                          <PlusCircle className="h-6 w-6 stroke-emerald-400" />
                        )}
                        {tx.type === "spend" && (
                          <CreditCard className="h-6 w-6 stroke-red-600" />
                        )}
                        {tx.type === "transfer" && (
                          <Send className="h-6 w-6 stroke-yellow-500" />
                        )}
                      </div>
                      ...
                      <div className="flex flex-col justify-center space-y-1 flex-grow">
                        <div className="flex items-center gap-2">
                          ...
                          <Copy
                            className="h-4 w-4 cursor-pointer text-[var(--color-secondary)]"
